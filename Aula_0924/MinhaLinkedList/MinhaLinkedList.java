...
public class MinhaLinkedList {
    ...
    public void add(int index, int element){
        ...
        //2º Cria o nodo a ser inserido
        MeuNodo aux = new MeuNodo();
        aux.valor=element;
        aux.proximo=null;

        //3º Confere assumirá o head
        if(index==0){
            //4º Novo nodo assumi o head
            aux.proximo=head;
            head=aux;
        }
        else{
            //4aº Percorro a lista até a posição anterior ao ponto de inserção
            MeuNodo nav=head;
